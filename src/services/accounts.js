...
export const useAccounts = create((set, get) => ({
  ...
  addAccount: () => {
    ...
    // Generate a new wallet/address for this account
    import('../lib/wallet/unifiedWallet.js').then(async ({ createUnifiedWallet }) => {
      const wallet = await createUnifiedWallet();
      const address = wallet?.fbx?.address || '';
      const list = [...accounts, { label, address }];
      saveAccounts(list);
      set({ accounts: list });
    });
  },
