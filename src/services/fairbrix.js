const rpcUrl = import.meta.env.VITE_FAIRBRIX_RPC_URL;
const rpcUser = import.meta.env.VITE_FAIRBRIX_RPC_USER || '';
const rpcPass = import.meta.env.VITE_FAIRBRIX_RPC_PASS || '';
const proxyUrl = import.meta.env.VITE_FAIRBRIX_PROXY_URL;
const broadcastUrl = import.meta.env.VITE_FAIRBRIX_BROADCAST_URL;
...
const dopelUrl = import.meta.env.VITE_DOPELS_API_URL || '';
const runesUrl = import.meta.env.VITE_RUNES_API_URL || '';

async function createOrdinalNFT({ name, description, content }) {
  if (!dopelUrl) throw new Error('VITE_DOPELS_API_URL missing');
  const res = await fetch(`${dopelUrl}/inscribe`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ name, description, content }) });
  if (!res.ok) throw new Error('Dopels API error');
  return res.json();
}
