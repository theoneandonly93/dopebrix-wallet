...
export default function Create() {
  ...
  // Etch form state (dope)
  const [dopeName, setDopeName] = useState('UNCOMMON•GOODS');
  const [dopeSymbol, setDopeSymbol] = useState('');
  ...
  // NFT form state
  // Dopel form state
  const [dopelName, setDopelName] = useState('My Fairbrix Dopel');
  const [dopelDesc, setDopelDesc] = useState('Description');
  const [dopelContent, setDopelContent] = useState('ipfs://…');
  ...
  // Map to current backend: ticker/dopeName, supply, decimals
  ...
  setStatus('Submitting Dope etch…');
    try {
      const res = await fairbrix.createRuneToken({ ticker: dopeName, supply, decimals });
  ...
  setStatus('Submitting Dope transfer…');
    try {
  ...
  const handleCreateDopel = async () => {
    setStatus('Creating Dopel…');
    try {
      const res = await fairbrix.createOrdinalNFT({ name: dopelName, description: dopelDesc, content: dopelContent });
      setStatus(res?.message || 'Dopel creation submitted');
    } catch (e) {
      setStatus(e?.message || 'Dopel creation failed');
    }
  };
    ...
    <div className="grid">
      <div className="card create-card">
        <div className="tabbar">
          <button className={tab==='token'?'active':''} onClick={() => setTab('token')}>Dopebrix Dopes</button>
          <button className={tab==='nft'?'active':''} onClick={() => setTab('nft')}>Dopels</button>
        </div>

        {tab === 'token' && (
          <div style={{marginTop:12}}>
            {/* Popular Dopes */}
            <div className="h-section">Popular Dopes</div>
            ...
            <div className="searchbar">
              ...
              <input placeholder="Search Dope" value={query} onChange={(e)=>setQuery(e.target.value)} />
            </div>
            <div className="spacer"/>
            <div className="cards-x">
              {(filtered || []).map((it, i) => (
                <div className="rune-card" key={i}>
                  <div className="rune-title">{(it.name || it.ticker || 'DOPE').toUpperCase()}</div>
                  <div className="rune-bar"/>
                  <div className="spacer"/>
                  <div className="muted">Dope number</div>
                  ...
                  <div className="spacer"/>
                  <button className="cta" onClick={()=>fairbrix.mintRune({ ticker: it.ticker || it.name || 'DOPE', amount:1 })} style={{width:'100%', background:'#eee', color:'#111'}}>Mint →</button>
                </div>
              ))}
            </div>
            ...
            {/* Etch your own */}
            <div className="h-title">Etch your Dope</div>
            <div className="spacer"/>
            <div style={{fontWeight:700, marginBottom:6}}>Your Dope</div>
            <label className="label">Dope name</label>
            <input className="input" value={dopeName} onChange={(e)=>setDopeName(e.target.value)} />
            ...
            <div className="spacer"/>
            <label className="label">Dope symbol (optional)</label>
            <input className="input" value={dopeSymbol} onChange={(e)=>setDopeSymbol(e.target.value)} />
            <div className="spacer"/>
            ...
            <div className="help">Set your desired fee rate for the Dope etch transactions.</div>
            <div className="spacer"/>
            ...
            <div className="alert"><div style={{fontWeight:800}}>Important</div>
              <div>We recommend setting a high enough fee rate for your order to be processed fast, especially if trying to etch a popular Dope name, to prevent front‑running.</div>
            </div>

            <div className="divider"/>
            {/* Simple Transfer */}
            <div className="h-section">Transfer a Dope</div>
            <div className="spacer"/>
            <label className="label">Dope symbol</label>
        ...
        {tab === 'nft' && (
          <div style={{marginTop:12}}>
            <label className="label">Dopel Name</label>
            <input className="input" value={dopelName} onChange={(e)=>setDopelName(e.target.value)} />
            <div className="spacer"/>
            <label className="label">Dopel Description</label>
            <input className="input" value={dopelDesc} onChange={(e)=>setDopelDesc(e.target.value)} />
            <div className="spacer"/>
            <label className="label">Dopel Content URI</label>
            <input className="input" value={dopelContent} onChange={(e)=>setDopelContent(e.target.value)} />
            <div className="spacer"/>
            <button className="cta primary" onClick={handleCreateDopel}>Create Dopel</button>
          </div>
        )}
      </div>
