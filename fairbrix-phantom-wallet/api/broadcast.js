...
export default async function handler(req, res) {
  ...
  const hex = body?.hex;
  if (!hex || typeof hex !== 'string') return bad(res, 400, 'Missing hex', req.headers.origin || '*');

  try {
    const result = await upstreamFetch({ jsonrpc: '2.0', id: Date.now().toString(36), method: 'sendrawtransaction', params: [hex] });
    return ok(res, { ok: true, txid: result?.result || result?.txid || '' }, req.headers.origin || '*');
  } catch (e) {
