...
export default function Wallet() {
  ...
  const home = useMemo(() => tab === 'home', [tab]);
  ...
  const load = async () => {
    // Balance
    let bal = 0;
    try { bal = Number(await fairbrix.getBalance()) || 0; } catch {}
    setBalance(bal);
    // Price
    try { const p = await fairbrix.getFbxUsdPrice(); setPriceUsd(Number(p) || 0); } catch {}

    // Address
    let addr = await resolveAddress();
    setAddress(addr);
    // Validate (best-effort via node/proxy)
    try {
      if (addr) {
        const v = await fairbrix.validateAddress(addr);
        if (v?.isvalid === true) setAddrValid('valid');
        else if (v?.error) setAddrValid(null); // unknown/unavailable; don't mark invalid on offline
        else setAddrValid('invalid');
      } else {
        setAddrValid(null);
      }
    } catch { setAddrValid(null); }
    ...
    try {
      ...
      const list = [{ symbol: 'FBX', name: 'Fairbrix', amount: bal }];
      setHoldings(list); setTokens(list);
    }
    ...
    <div className="grid">
      <div className="card">
        ...
        {(() => {
          ...
          return (
            <>
              <div className="balance">{`${fmtAmt(val)} ${primaryAsset}`}</div>
              <div className="muted" style={{marginTop:4}}>{fmtUSD(val * usd)}</div>
            </>
          );
        })()}
        ...
        <div className="addr-row" style={{marginTop:12}}>
          <span className="pill mono address-pill" title="Your receiving address">{address || 'â€”'}</span>
          {addrValid === 'valid' && <span className="pill" style={{borderColor:'#1f3a2a', color:'#c9f3db'}}>Valid</span>}
          {addrValid === 'invalid' && <span className="pill" style={{borderColor:'#3a1f1f', color:'#f3c9c9'}}>Not valid on this node</span>}
        </div>
        <div className="meta-row" style={{marginTop:8}}>
          <span className="pill mono">{height ? `Height #${height}` : 'Height â€”'}</span>
        </div>
        <div className="actions">
          <Action label="Send" onClick={() => setSendSheet(true)} icon={<svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M4 12h12l-4-4 1.4-1.4L20.8 14l-7.4 7.4L12 20l4-4H4z"/></svg>} />
          <Action label="Receive" onClick={() => setReceiveSheet('menu')} icon={<svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M20 3H4a1 1 0 0 0-1 1v4h2V5h14v14h-3v2h4a1 1 0 0 0 1-1V4a1 1 0 0 0-1-1zM3 16H1l4-6 4 6H7v5H5v-5z"/></svg>} />
          <Action label="Buy" onClick={()=>window.__setAppTab && window.__setAppTab('buy')} icon={<svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2S15.9 22 17 22s2-.9 2-2-.9-2-2-2zM7.2 14h9.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49-1.74-.99-3.58 6.49H8.1L5.25 2H1v2h2l3.6 7.59-1.35 2.44C4.52 15.37 5.48 17 7 17h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.1L7.2 14z"/></svg>} />
        </div>
        <div className="actions">
          <Action label="Stake" onClick={handleStake} icon={<svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2 2 7l10 5 10-5-10-5zm0 7L2 4v13l10 5 10-5V4l-10 5z"/></svg>} />
          <Action label="Swap" onClick={()=>window.__setAppTab && window.__setAppTab('swap')} icon={<svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M4 12h12l-4-4 1.4-1.4L20.8 14l-7.4 7.4L12 20l4-4H4z"/></svg>} />
          <div className="action" onClick={() => window.location.reload()}>
            <svg width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12 6V3L8 7l4 4V8c2.76 0 5 2.24 5 5a5 5 0 0 1-5 5 5 5 0 0 1-4.9-4h-2.02A7 7 0 0 0 12 22a7 7 0 0 0 7-7c0-3.87-3.13-7-7-7z"/></svg>
            <div>Refresh</div>
          </div>
        </div>
      </div>
          ...
          {holdings && holdings.map((t, i) => (
            <div className="list-item" key={`${t.symbol}-${i}`}>
              <div style={{display:'flex', alignItems:'center', gap:10}}>
                <div style={{width:28,height:28,borderRadius:8,background:'#1c1f23',display:'inline-flex',alignItems:'center',justifyContent:'center',fontWeight:800}}>
                  {t.symbol?.[0] || 'F'}
                </div>
                <div>
            ...
            {txs.map((t, i) => (
              <div key={t.txid || i} className="list-item">
                ...
                <div style={{textAlign:'right'}}>
                  <div style={{fontWeight:600}}>{t.amount ? `${fmtAmt(t.amount)} FBRX` : ''}</div>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="list">
                ...
                <div className="list-item">
                  <div style={{flex:1}}>
                    ...
                    <div className="muted" style={{fontSize:12, marginTop:6}}>Available: {fmtAmt(balance)} FBX â€¢ {fmtUSD((Number(balance)||0)*(Number(priceUsd)||0))}</div>
                  </div>
                  <div style={{display:'flex',alignItems:'flex-end'}}>
                    <button className="pill" onClick={()=>{ const max = (sendAsset && sendAsset.symbol !== 'FBX') ? (Number(sendAsset.amount)||0) : (Number(balance)||0); setAmount(String(max)); }} style={{whiteSpace:'nowrap'}}>Max</button>
                  </div>
                </div>
                <div className="row" style={{marginTop:8}}>
                  ...
                  }} disabled={!(Number(amount)>0 && String(to||'').length>20) || addrCheck === 'invalid'}>Send</button>
                </div>
              </div>
            )}
          </div>
            ...
            <div className="list">
              ...
              <div className="list-item">
                <div className="row" style={{width:'100%'}}>
                  ...
                  <button className="cta primary" disabled={sending} onClick={()=>{ setConfirmErr(''); setSignErr(''); setSignPass(''); setSignSheet(true); }}>{sending ? "Sending…" : "Approve & Send"}</button>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="row" style={{width:'100%'}}>
                  ...
                  <button className="cta primary" disabled={signBusy} onClick={async()=>{
                    ...
                    try {
                      ...
                      try {
                        const pf = await fairbrix.preflightSend({ amount: Number(amount)||0, fromAddress: address||'' });
                        if (!pf?.ok) { setSignErr(pf?.message || 'Preflight failed'); setSignBusy(false); return; }
                      } catch (e) { /* proceed to attempt send; handleSend will surface any errors */ }
                      const res = await handleSend();
                      if (res?.ok) { setConfirmSheet(false); setSendForm(false); setSendSheet(false); setSignSheet(false); setConfirmErr(''); setSignErr(''); }
                      else { setConfirmErr(res?.error || 'Failed to send'); setSignErr(res?.error || 'Failed to send'); }
            ...
            <div className="list">
              ...
              <div className="list-item">
                <div>
                  ...
                  <div className="muted" style={{fontSize:12}}>Receive Ordinals-style NFTs.</div>
                </div>
                <div className="muted">â€º</div>
              </div>
            </div>
